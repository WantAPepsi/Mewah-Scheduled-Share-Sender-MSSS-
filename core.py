...
async def provider_diagnostic():
    ...
    async with httpx.AsyncClient(timeout=25, follow_redirects=True, headers={"User-Agent":"JARVIS-Mewah-Share-Monitor/10.0"}) as client:
        for path, params in [
...
async def _fetch_marketstack(c):
    ...
    ds = c.get("data_source", {})
    target_mic = str(ds.get("marketstack_mic") or "XSES").strip().upper()
    resolved_symbol = str(ds.get("marketstack_resolved_symbol") or "").strip()
    if not resolved_symbol:
        raise RuntimeError(
            "Marketstack provider symbol has not been verified yet. Click 'Test Marketstack coverage' first. "
            "JARVIS will only save a symbol after Marketstack returns XSES/SGX EOD rows."
        )
